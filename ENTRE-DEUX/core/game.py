...
# ─────────────────────────────────────────────────────────────────────────────
#  MIXINS : groupes de méthodes externalisés dans core/_game_*_mixin.py
# ─────────────────────────────────────────────────────────────────────────────
#  La classe Game est très grosse. Pour rester sous la limite de 500 lignes
#  par fichier (et pour pouvoir relire facilement chaque domaine), on a
#  extrait certains groupes de méthodes dans des fichiers séparés sous forme
#  de MIXINS (= classes destinées à être héritées). Game hérite de tous ces
#  mixins → leurs méthodes restent accessibles via self.X comme avant.
from core._game_audio_mixin       import AudioMixin
from core._game_checkpoint_mixin  import CheckpointMixin
from core._game_boss_ui_mixin     import BossUIMixin
from core._game_cinematique_mixin import CinematiqueMixin
from core._game_save_mixin        import SaveMixin
from core._game_debug_panel_mixin import DebugPanelMixin
from core._game_preload_mixin     import PreloadMixin


class Game(AudioMixin, CheckpointMixin, BossUIMixin,
           CinematiqueMixin, SaveMixin, DebugPanelMixin,
           PreloadMixin):
    ...
    # ─── Story flags debug panel (F5) → DebugPanelMixin ──────────
    #   _story_flags_panel_handle_key, _handle_textinput,
    #   _dessiner_story_flags_panel

    # ─── _declencher_cinematique_mort() → CinematiqueMixin ──────────────
    ...
    # ─── _appliquer_event_pnj() + sous-handlers → CinematiqueMixin ───
    # ─── Cinématiques conditionnelles (auto-fire par flag),
    #     _lancer_cinematique_par_nom, _skipper_cinematique,
    #     _reset_compteur_cinematique, _tester_cinematique
    #     → CinematiqueMixin
    ...
    # ─── _construire_save_data, _appliquer_save_data,
    #     _charger_partie, _sauvegarder → SaveMixin
    ...
    # ─── CHECKPOINTS / AUTOSAVE → CheckpointMixin ───────────────────────
    #   _compter_pommes_inventaire, _garantir_minimum_pommes,
    #   _snapshot_player, _verifier_autosave_zones, _restaurer_checkpoint,
    #   _respawn_meme_salle  → core/_game_checkpoint_mixin.py

    # ─── BARRE DE VIE BOSS → BossUIMixin ────────────────────────────────
    #   _trouver_boss_actif, _dessiner_boss_hp_bar
    #   → core/_game_boss_ui_mixin.py
    ...
    # ─── Pré-chargement textures (au chgt de map) → PreloadMixin ────
    #   _precharger_textures, _precharger_textures_legacy,
    #   _precharger_textures_OLD
